...
namespace Vivit_Control_Center.Views.Modules
{
    public partial class SshModule : BaseSimpleModule
    {
        ...
        private void Cleanup(){ try { _readCts?.Cancel(); } catch { } try { (_shell as IDisposable)?.Dispose(); } catch { } try { (_client as IDisposable)?.Dispose(); } catch { } try { _sshInput?.Dispose(); } catch { } try { _sshProcess?.Dispose(); } catch { } _readCts=null; _shell=null; _client=null; _sshInput=null; _sshProcess=null; _mode=SshMode.None; _connected=false; }
        private void ReadShellLoop((Type SshClient, Type ShellStream, MethodInfo SshClient_Get_Connect, MethodInfo SshClient_Get_CreateShellStream, MethodInfo ShellStream_Read, MethodInfo ShellStream_WriteLine) types, CancellationToken token){ try { var buffer=new byte[4096]; while(!token.IsCancellationRequested && _shell!=null){ int read=(int)types.ShellStream_Read.Invoke(_shell,new object[]{buffer,0,buffer.Length}); if(read>0){ var text=Encoding.UTF8.GetString(buffer,0,read); Dispatcher.Invoke(()=>AppendOutput(text,Colors.White)); } else Thread.Sleep(10); } } catch { } }
        ...
        private void SendCurrentInput(){ if(!_connected) return; var text=txtInput.Text; txtInput.Clear(); try { if(_mode==SshMode.SshNet){ var shellType=_shell?.GetType(); var write=shellType?.GetMethod("WriteLine", BindingFlags.Public|BindingFlags.Instance, null, new[]{typeof(string)}, null); write?.Invoke(_shell,new object[]{text}); } else if(_mode==SshMode.SshExe){ _sshInput?.WriteLine(text); _sshInput?.Flush(); } } catch(Exception ex){ AppendOutput(string.Format(LocalizationManager.GetString("SSH.SendError","Send error: {0}"), ex.Message)+"\n", Colors.Red); } }
